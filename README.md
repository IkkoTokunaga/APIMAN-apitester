...
本イメージは **`apiman/api-tester:latest`** として Docker Hub に配布する想定です。
...
### 8.2 メンテナ向け（Docker Hub に push する）

#### 8.2.1 事前準備

1. [Docker Hub](https://hub.docker.com/) でリポジトリを作成（例: `apiman/api-tester`）。
2. ローカルから Docker Hub にログイン。
   ```bash
   docker login
   ```
3. 機密情報がイメージに混入していないか `.dockerignore` を確認（`.env` / `database.sqlite` などが除外されていること）。

#### 8.2.2 シングルアーキテクチャで push（最速）

ローカルが `linux/amd64` で、同じアーキの利用者に配るだけなら最小構成で十分です。

```bash
# ビルド（compose の image タグがそのまま使われる）
docker compose build

# Docker Hub に push
docker push apiman/api-tester:latest
```

#### 8.2.3 マルチアーキ (amd64 + arm64) で push（推奨）

Apple Silicon (M シリーズ) / Raspberry Pi などでも動かせるよう、`buildx` でマルチアーキイメージを作成します。

```bash
# 初回のみ: buildx ビルダーを作成
docker buildx create --name apiman-builder --use
docker buildx inspect --bootstrap

# バージョンタグとともに一括ビルド & push
VERSION=1.0.0
docker buildx build \
  --platform linux/amd64,linux/arm64 \
  -t apiman/api-tester:latest \
  -t apiman/api-tester:${VERSION} \
  --push \
  .
```

> `--push` を付けるとビルド成果物が直接 Docker Hub に送られます（ローカルには残りません）。
> 手元で動作確認したい場合は `--push` の代わりに `--load` を指定し、単一アーキでビルドしてください。
...
リリース時は `latest` と `X.Y.Z` の 2 つを同時に push するのが定番です。
...
docker rmi apiman/api-tester:latest
docker compose -f docker-compose.dist.yml pull
docker compose -f docker-compose.dist.yml up -d
